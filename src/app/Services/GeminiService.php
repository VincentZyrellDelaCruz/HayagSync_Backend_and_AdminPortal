...
";
        ...
        $response = Gemini::generativeModel(model: config('gemini.model', 'gemini-2.5-flash'))
    ...
    /*
     * Used only when real Gemini urgency assessment is enabled.
     *
     * The normal local-development path does NOT call this method when
     * GEMINI_DUMMY_MODE=true; it uses TraditionalUrgencyAnalyzer instead.
     */
    public function generateUrgencyAssessment(string $title, string $description, string $category, ?string $location = null): ?bool
    {
        $prompt = <<<PROMPT
You are a school incident safety triage assistant supporting New Era University Integrated School.

Determine whether the provided incident report indicates an IMMEDIATE or SERIOUS SAFETY CONCERN that should bypass normal Section Adviser handling and be escalated to the Ministrong Tagasubaybay.

The report may be written in English, Filipino, or a mixture of both languages.

Return ONLY one lowercase value:
...
Important rules:
- Do not treat emotional or exaggerated language by itself as urgent.
- Consider concrete safety indicators.
- Consider weapons, serious physical injury, credible threats, sexual violence, self-harm, severe coercion, blackmail involving intimate material, stalking, or other serious immediate risks.
- Do not diagnose mental health conditions.
- Do not infer motives.
- Do not assume that a student is guilty.
- Do not make legal conclusions.
- Do not treat ordinary teasing, insults, exclusion, arguments, embarrassment, or non-threatening conflict as automatically urgent.
- If the report lacks sufficient evidence of an immediate or serious safety concern, return false.
...
PROMPT;
        ...
        $value = trim(mb_strtolower($response->text()));
        $value = preg_replace('/^```(?:text)?\s*|\s*```$/i', '', $value);
    ...
    // WILL BE REMOVED BEFORE COMPLETION AND PRODUCTION
    ...
    // WILL BE REMOVED BEFORE COMPLETION AND PRODUCTION
    public function generateDummyEvidenceVerification(string $fileType, string $fileName): array
    {
        return [
            'state' => 'Unverified',
            'details' => 'Temporary development result. AI evidence verification is currently disabled and no Gemini request was made.',
        ];
    }

    // WILL BE REMOVED BEFORE COMPLETION AND PRODUCTION
    public function generateDummyUrgencyAssessment(string $title, string $description, string $category, ?string $location = null): bool
    {
        // Legacy dummy method retained for compatibility. ReportUrgencyService no longer uses this for urgency decisions.
        return false;
    }
}
